...
function parseFountain(input) {
   ...
   function extractScenesFromText(text) {
    ...
    if (!text || !text.trim()) {
        console.warn('No text to extract scenes from');
        return [];
    }
    ...
    tokens.forEach(token => {
        if (token.type === 'sceneheading') {
            ...
            console.log('Created scene:', currentScene.number, currentScene.heading);
   ...
   function switchView(view) {
    ...
    if (view === 'script') {
        ...
        // CRITICAL FIX: Force scene extraction before rendering cards
        if (fountainInput && fountainInput.value.trim()) {
            projectData.projectInfo.scenes = extractScenesFromText(fountainInput.value);
            console.log('Extracted scenes for card view:', projectData.projectInfo.scenes.length);
        }
